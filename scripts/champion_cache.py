...
CLI usage:
    python3 scripts/champion_cache.py get --cache-dir <dir> --champion-content-file <path> --case-id <id> [--agent-content-files <path> ...]
    python3 scripts/champion_cache.py put --cache-dir <dir> --champion-content-file <path> --case-id <id> --result '<json>' [--agent-content-files <path> ...]
...
class ChampionCache:
    ...
    def _cache_key(self, champion_content: str, case_id: str, agent_contents: list[str] | None = None) -> str:
        parts = [champion_content]
        if agent_contents:
            parts.extend(sorted(agent_contents))
    ...
    def get(self, champion_content: str, case_id: str, agent_contents: list[str] | None = None) -> ExecutionResult | None:
        path = self._cache_path(self._cache_key(champion_content, case_id, agent_contents))
        try:
    ...
    def put(self, champion_content: str, case_id: str, result: ExecutionResult, agent_contents: list[str] | None = None) -> None:
...
def main() -> None:
    ...
    get_cmd.add_argument("--agent-content-files", nargs="*", default=[], help="Paths to agent .md files to include in cache key")
    ...
    put_cmd.add_argument("--agent-content-files", nargs="*", default=[], help="Paths to agent .md files to include in cache key")
    ...
    agent_contents = [Path(p).read_text() for p in args.agent_content_files] if hasattr(args, "agent_content_files") and args.agent_content_files else None

    if args.command == "get":
        content = Path(args.champion_content_file).read_text()
        result = cache.get(content, args.case_id, agent_contents)
        ...
        cache.put(content, args.case_id, result, agent_contents)
